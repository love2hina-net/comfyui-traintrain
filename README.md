...
- [Testing](#testing)
- [NOTICE](#notice)
- [Difference as original(Stable Diffusion WebUI Version)](#difference-as-originalstable-diffusion-webui-version)
...
## Difference as original(Stable Diffusion WebUI Version)
### deterioration
- Need specifination a model type for the selected checkpoint(model).
- Not able to add a task to works queue in learning.
- There is no function to output an image for confirmation prompts at iLECO configuration pane.

### enhancement
- Was able to select a log file for graph the progress of learning on GUI.
...
## Queue
You can reserve learning. Pressing the `Add to Queue` button reserves learning with the current settings. If pressed before learning, after learning ends with the settings when `Start Training` was pressed, the learning in the Queue list is processed in order. You cannot add settings with the same `save lora name`.
